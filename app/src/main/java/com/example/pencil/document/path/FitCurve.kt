package com.example.pencil.document.path

import android.text.TextUtils
import kotlin.math.floor
import kotlin.math.hypot


fun pathFitCurve(pathString: String, maxError: Int): String {
    var pointsList : MutableList<MutableList<Double>> = mutableListOf()
    var pathList = TextUtils.split(pathString, " ")

    var i = 0
    while (i < pathList.size) {
        when (pathList[i]) {
            "M" -> {
                pointsList.add(mutableListOf(pathList[i + 1].toDouble(), pathList[i + 2].toDouble()))
                i += 2
            }
            "L" -> {
                pointsList.add(mutableListOf(pathList[i + 1].toDouble(), pathList[i + 2].toDouble()))
                i += 2
            }
            "Q" -> {
                pointsList.add(mutableListOf(pathList[i + 3].toDouble(), pathList[i + 4].toDouble()))
                i += 4
            }
        }
        i++
    }

    var bezierCurveList = fitCurve(pointsList, maxError)
    var pathTemp = ""

    /*path.moveTo(bezierCurveList[0][0][0].toFloat(), bezierCurveList[0][0][1].toFloat())*/
    pathTemp += "M " +
            bezierCurveList[0][0][0].toFloat() + " " + bezierCurveList[0][0][1].toFloat() + " "

    if (bezierCurveList[0].size > 1) {
        for (bezierCurve in bezierCurveList) {
            //path.lineTo(bezierCurve[3][0].toFloat(), bezierCurve[3][1].toFloat())
            pathTemp += "C " +
                    bezierCurve[1][0].toFloat() + " " + bezierCurve[1][1].toFloat() + " " +
                    bezierCurve[2][0].toFloat() + " " + bezierCurve[2][1].toFloat() + " " +
                    bezierCurve[3][0].toFloat() + " " + bezierCurve[3][1].toFloat() + " "
            /*path.cubicTo(
                bezierCurve[1][0].toFloat(), bezierCurve[1][1].toFloat(),
                bezierCurve[2][0].toFloat(), bezierCurve[2][1].toFloat(),
                bezierCurve[3][0].toFloat(), bezierCurve[3][1].toFloat()
            )*/
        }
    }

    return pathTemp
}

/**
 * Fit one or more Bezier curves to a set of points.
 *
 * @param {Array<Array<Number>>} points - Array of digitized points, e.g. [[5,5],[5,50],[110,140],[210,160],[320,110]]
 * @param {Number} maxError - Tolerance, squared error between points and fitted curve
 * @returns {Array<Array<Array<Number>>>} Array of Bezier curves, where each element is [first-point, control-point-1, control-point-2, second-point] and points are [x, y]
 */
fun fitCurve(points: MutableList<MutableList<Double>>, maxError: Int): MutableList<MutableList<MutableList<Double>>> {
    // Remove duplicate points
    var points = points.filterIndexed{ i, point ->
        i == 0 || points[i] != points[i-1]
    }.toMutableList()
    if(points.size < 2){
        return mutableListOf(mutableListOf(mutableListOf(points[0][1], points[0][1])))
    }

    val len = points.size
    val leftTangent = createTangent(points[1], points[0])
    val rightTangent = createTangent(points[len - 2], points[len - 1])

    return fitCubic(points, leftTangent, rightTangent, maxError)
}

/**
 * Fit a Bezier curve to a (sub)set of digitized points.
 * Your code should not call this function directly. Use {@link fitCurve} instead.
 *
 * @param {Array<Array<Number>>} points - Array of digitized points, e.g. [[5,5],[5,50],[110,140],[210,160],[320,110]]
 * @param {Array<Number>} leftTangent - Unit tangent vector at start point
 * @param {Array<Number>} rightTangent - Unit tangent vector at end point
 * @param {Number} error - Tolerance, squared error between points and fitted curve
 * @returns {Array<Array<Array<Number>>>} Array of Bezier curves, where each element is [first-point, control-point-1, control-point-2, second-point] and points are [x, y]
 */
fun fitCubic(points: MutableList<MutableList<Double>>, leftTangent: MutableList<Double>, rightTangent: MutableList<Double>, error: Int): MutableList<MutableList<MutableList<Double>>> {
    val MaxIterations = 20  //Max times to try iterating (to find an acceptable curve)

    //Control points of fitted Bezier curve
    var bezCurve: MutableList<MutableList<Double>>
    //Parameter values for point
    var u: MutableList<Double>
    //Improved parameter values
    var uPrime: MutableList<Double>
    var maxError: Double
    //Maximum fitting error
    var prevErr: Double
    var splitPoint: Double
    //Point to split point set at if we need more than one curve
    var prevSplit: Double
    var centerVector: MutableList<Double>
    var toCenterTangent: MutableList<Double>
    //Unit tangent vector(s) at splitPoint
    var fromCenterTangent: MutableList<Double>
    //Array of fitted Bezier curves if we need more than one curve
    var beziers: MutableList<MutableList<MutableList<Double>>>
    var dist: Double

    //Use heuristic if region only has two points in it
    if (points.size == 2) {
        dist = vectorLen(subtract(points[0], points[1])) / 3.0;
        bezCurve = mutableListOf(
            points[0],
            addArrays(points[0], mulItems(leftTangent,  dist)),
            addArrays(points[1], mulItems(rightTangent, dist)),
            points[1]
        )
        return mutableListOf(bezCurve)
    }

    //Parameterize points, and attempt to fit curve
    u = chordLengthParameterize(points)
    var list = generateAndReport(points, u, u, leftTangent, rightTangent)
    bezCurve = list[0] as MutableList<MutableList<Double>>
    maxError = list[1] as Double
    splitPoint = list[2] as Double

    if ((maxError == 0.0) || (maxError < error)) {
        return mutableListOf(bezCurve)
    }
    //If error not too large, try some reparameterization and iteration
    if (maxError < (error*error)) {

        uPrime = u
        prevErr = maxError
        prevSplit = splitPoint

        for (i in 0 until MaxIterations) {

            uPrime = reparameterize(bezCurve, points, uPrime).toMutableList()
            var list = generateAndReport(points, u, uPrime, leftTangent, rightTangent)
            bezCurve = list[0] as MutableList<MutableList<Double>>
            maxError = list[1] as Double
            splitPoint = list[2] as Double

            if (maxError < error) {
                return mutableListOf(bezCurve)
            }
            //If the development of the fitted curve grinds to a halt,
            //we abort this attempt (and try a shorter curve):
            else if(splitPoint == prevSplit) {
                val errChange = maxError/prevErr
                if((errChange > .9999) && (errChange < 1.0001)) {
                    break;
                }
            }

            prevErr = maxError
            prevSplit = splitPoint
        }
    }

    //Fitting failed -- split at max error point and fit recursively
    beziers = mutableListOf()

    //To create a smooth transition from one curve segment to the next, we
    //calculate the line between the points directly before and after the
    //center, and use that as the tangent both to and from the center point.
    centerVector = subtract(points[(splitPoint-1).toInt()], points[(splitPoint+1).toInt()])
    //However, this won't work if they're the same point, because the line we
    //want to use as a tangent would be 0. Instead, we calculate the line from
    //that "double-point" to the center point, and use its tangent.
    if(centerVector.all{value -> value == 0.0}) {
        //[x,y] -> [-y,x]: http://stackoverflow.com/a/4780141/1869660
        centerVector = subtract(points[(splitPoint-1).toInt()], points[splitPoint.toInt()])

        var temp = centerVector[0]
        centerVector[0] = -centerVector[1]
        centerVector[1] = temp
    }
    toCenterTangent = normalize(centerVector);
    //To and from need to point in opposite directions:
    fromCenterTangent = mulItems(toCenterTangent, -1.0);

    /*
    Note: An alternative to this "divide and conquer" recursion could be to always
          let new curve segments start by trying to go all the way to the end,
          instead of only to the end of the current subdivided polyline.
          That might let many segments fit a few points more, reducing the number of total segments.
          However, a few tests have shown that the segment reduction is insignificant
          (240 pts, 100 err: 25 curves vs 27 curves. 140 pts, 100 err: 17 curves on both),
          and the results take twice as many steps and milliseconds to finish,
          without looking any better than what we already have.
    */
    beziers = (beziers + (fitCubic(points.slice(0..(splitPoint).toInt()) as MutableList<MutableList<Double>>, leftTangent, toCenterTangent, error))) as MutableList<MutableList<MutableList<Double>>>
    beziers = (beziers + (fitCubic(points.slice((splitPoint).toInt()..points.lastIndex) as MutableList<MutableList<Double>>, fromCenterTangent, rightTangent, error))) as MutableList<MutableList<MutableList<Double>>>
    return beziers
};

fun generateAndReport(points: MutableList<MutableList<Double>>, paramsOrig: MutableList<Double>, paramsPrime: MutableList<Double>, leftTangent: MutableList<Double>, rightTangent: MutableList<Double>): MutableList<Any> {
    var bezCurve: MutableList<MutableList<Double>>
    var maxError: Double
    var splitPoint: Double

    bezCurve = generateBezier(points, paramsPrime, leftTangent, rightTangent)
    //Find max deviation of points to fitted curve.
    //Here we always use the original parameters (from chordLengthParameterize()),
    //because we need to compare the current curve to the actual source polyline,
    //and not the currently iterated parameters which reparameterize() & generateBezier() use,
    //as those have probably drifted far away and may no longer be in ascending order.
    var list = computeMaxError(points, bezCurve, paramsOrig)
    maxError = list[0]
    splitPoint = list[1]

    /*if(progressCallback) {
        progressCallback({
                bez: bezCurve,
                points: points,
                params: paramsOrig,
                maxErr: maxError,
                maxPoint: splitPoint,
        });
    }*/

    return mutableListOf(bezCurve, maxError, splitPoint)
}

/**
 * Use least-squares method to find Bezier control points for region.
 *
 * @param {Array<Array<Number>>} points - Array of digitized points
 * @param {Array<Number>} parameters - Parameter values for region
 * @param {Array<Number>} leftTangent - Unit tangent vector at start point
 * @param {Array<Number>} rightTangent - Unit tangent vector at end point
 * @returns {Array<Array<Number>>} Approximated Bezier curve: [first-point, control-point-1, control-point-2, second-point] where points are [x, y]
 */
fun generateBezier(points: MutableList<MutableList<Double>>, parameters: MutableList<Double>, leftTangent: MutableList<Double>, rightTangent: MutableList<Double>): MutableList<MutableList<Double>> {
    //Precomputed rhs for eqn
    var A = zeros_Xx2x2(parameters.size) //Compute the A's
    var a = zeros_Xx2x2(parameters.size)
    //Determinants of matrices
    var det_C0_C1: Double
    var det_C0_X: Double
    var det_X_C1: Double
    //Alpha values, left and right
    var alpha_l: Double
    var alpha_r: Double

    var epsilon: Double
    var segLength: Double

    var tmp : MutableList<Double>
    var u : Double
    var ux : Double
    var firstPoint = points[0]
    var lastPoint = points[points.size-1]

    //Bezier curve ctl pts
    var bezCurve = mutableListOf(firstPoint, mutableListOf(), mutableListOf(), lastPoint)
    //console.log('gb', parameters.length)

    var len = parameters.size
    for (i in 0 until len) {
        u = parameters[i]
        ux = 1 - u
        a[i] = A[i]
        // TODO: 21/11/2021 Aggiunto a[i]

        a[0] = mulItems(leftTangent,  3.0 * u  * (ux*ux))
        a[1] = mulItems(rightTangent, 3.0 * ux * (u*u))
    }

    //Matrices C & X
    //Create the C and X matrices
    var C = mutableListOf(mutableListOf(0.0,0.0), mutableListOf(0.0,0.0))
    var X = mutableListOf(0.0,0.0)
    len = points.size
    for (i in 0 until len) {
        u = parameters[i]
        a[i] = A[i]

        C[0][0] += dot(a[0], a[0])
        C[0][1] += dot(a[0], a[1])
        C[1][0] += dot(a[0], a[1])
        C[1][1] += dot(a[1], a[1])

        tmp = subtract(points[i], q(mutableListOf(firstPoint, firstPoint, lastPoint, lastPoint), u))

        X[0] += dot(a[0], tmp)
        X[1] += dot(a[1], tmp)
    }

    //Compute the determinants of C and X
    det_C0_C1 = (C[0][0] * C[1][1]) - (C[1][0] * C[0][1]);
    det_C0_X  = (C[0][0] * X[1]   ) - (C[1][0] * X[0]   );
    det_X_C1  = (X[0]    * C[1][1]) - (X[1]    * C[0][1]);

    //Finally, derive alpha values
    alpha_l = if(det_C0_C1 == 0.0) 0.0 else det_X_C1 / det_C0_C1;
    alpha_r = if(det_C0_C1 == 0.0) 0.0 else det_C0_X / det_C0_C1;

    //If alpha negative, use the Wu/Barsky heuristic (see text).
    //If alpha is 0, you get coincident control points that lead to
    //divide by zero in any subsequent NewtonRaphsonRootFind() call.
    segLength = vectorLen(subtract(firstPoint, lastPoint))
    epsilon = 1.0e-6 * segLength
    if (alpha_l < epsilon || alpha_r < epsilon) {
        //Fall back on standard (probably inaccurate) formula, and subdivide further if needed.
        bezCurve[1] = addArrays(firstPoint, mulItems(leftTangent,  segLength / 3.0))
        bezCurve[2] = addArrays(lastPoint,  mulItems(rightTangent, segLength / 3.0))
    } else {
        //First and last control points of the Bezier curve are
        //positioned exactly at the first and last data points
        //Control points 1 and 2 are positioned an alpha distance out
        //on the tangent vectors, left and right, respectively
        bezCurve[1] = addArrays(firstPoint, mulItems(leftTangent,  alpha_l))
        bezCurve[2] = addArrays(lastPoint,  mulItems(rightTangent, alpha_r))
    }

    return bezCurve
}

/**
 * Given set of points and their parameterization, try to find a better parameterization.
 *
 * @param {Array<Array<Number>>} bezier - Current fitted curve
 * @param {Array<Array<Number>>} points - Array of digitized points
 * @param {Array<Number>} parameters - Current parameter values
 * @returns {Array<Number>} New parameter values
 */
fun reparameterize(bezier: MutableList<MutableList<Double>>, points: MutableList<MutableList<Double>>, parameters: MutableList<Double>): List<Double> {
    /*
    var j, len, point, results, u;
    results = [];
    for (j = 0, len = points.length; j < len; j++) {
        point = points[j], u = parameters[j];
        results.push(newtonRaphsonRootFind(bezier, point, u));
    }
    return results;
    */
    return parameters.mapIndexed { i, p -> newtonRaphsonRootFind(bezier, points[i], p) }
}

/**
* Use Newton-Raphson iteration to find better root.
*
* @param {Array<Array<Number>>} bez - Current fitted curve
* @param {Array<Number>} point - Digitized point
* @param {Number} u - Parameter value for "P"
* @returns {Number} New u
*/
fun newtonRaphsonRootFind(bez: MutableList<MutableList<Double>>, point: MutableList<Double>, u: Double): Double {
    /*
        Newton's root finding algorithm calculates f(x)=0 by reiterating
        x_n+1 = x_n - f(x_n)/f'(x_n)
        We are trying to find curve parameter u for some point p that minimizes
        the distance from that point to the curve. Distance point to curve is d=q(u)-p.
        At minimum distance the point is perpendicular to the curve.
        We are solving
        f = q(u)-p * q'(u) = 0
        with
        f' = q'(u) * q'(u) + q(u)-p * q''(u)
        gives
        u_n+1 = u_n - |q(u_n)-p * q'(u_n)| / |q'(u_n)**2 + q(u_n)-p * q''(u_n)|
    */

    var d = subtract(q(bez, u), point)
    var qprime = qprime(bez, u)
    var numerator = mulMatrix(d, qprime)
    var denominator = sum(squareItems(qprime)) + 2 * mulMatrix(d, qprimeprime(bez, u))

    if (denominator == 0.0) {
        return u
    } else {
        return u - (numerator/denominator)
    }
}

/**
* Assign parameter values to digitized points using relative distances between points.
*
* @param {Array<Array<Number>>} points - Array of digitized points
* @returns {Array<Number>} Parameter values
*/
fun chordLengthParameterize(points: MutableList<MutableList<Double>>): MutableList<Double> {
    var u = mutableListOf<Double>()
    var currU: Double
    var prevU = 0.0
    var prevP = mutableListOf<Double>()

    points.forEachIndexed{i, p ->
        currU = if(i != 0) prevU + vectorLen(subtract(p, prevP)) else 0.0
        u.add(currU)

        prevU = currU
        prevP = p
    }
    u = u.map{x -> x/prevU}.toMutableList()

    return u
}

/**
* Find the maximum squared distance of digitized points to fitted curve.
*
* @param {Array<Array<Number>>} points - Array of digitized points
* @param {Array<Array<Number>>} bez - Fitted curve
* @param {Array<Number>} parameters - Parameterization of points
* @returns {Array<Number>} Maximum error (squared) and point of max error
*/
fun computeMaxError(points: MutableList<MutableList<Double>>, bez: MutableList<MutableList<Double>>, parameters: MutableList<Double>): MutableList<Double> {
    //Current error
    var dist: Double
    //Vector from point to curve
    var v: MutableList<Double>
    var count = points.size
    var point: MutableList<Double>
    var t: Double

    //Maximum error
    var maxDist = 0.0

    //Point of maximum error
    var splitPoint = floor(points.size / 2.0)

    val t_distMap = mapTtoRelativeDistances(bez, 10)

    for (i in 0 until count) {
        point = points[i]
        //Find 't' for a point on the bez curve that's as close to 'point' as possible:
        t = find_t(bez, parameters[i], t_distMap, 10)

        v = subtract(q(bez, t), point)
        dist = v[0]*v[0] + v[1]*v[1]

        if (dist > maxDist) {
            maxDist = dist
            splitPoint = i.toDouble()
        }
    }

    return mutableListOf(maxDist, splitPoint)
};

//Sample 't's and map them to relative distances along the curve:
var mapTtoRelativeDistances = fun (bez: MutableList<MutableList<Double>>, B_parts: Int): MutableList<Double> {
    var B_t_curr = mutableListOf<Double>()
    var B_t_dist = mutableListOf<Double>(0.0)
    var B_t_prev = bez[0];
    var sumLen = 0.0

    for (i in 1..B_parts) {
      B_t_curr = q(bez, (i/B_parts).toDouble())

      sumLen += vectorLen(subtract(B_t_curr, B_t_prev))

      B_t_dist.add(sumLen)
      B_t_prev = B_t_curr
    }

    //Normalize B_length to the same interval as the parameter distances; 0 to 1:
    B_t_dist = B_t_dist.map{x -> x/sumLen}.toMutableList()
    return B_t_dist
}

fun find_t(bez: MutableList<MutableList<Double>>, param: Double, t_distMap: MutableList<Double>, B_parts: Int): Double {
    if(param < 0) { return 0.0 }
    if(param > 1) { return 1.0 }

    /*
        'param' is a value between 0 and 1 telling us the relative position
        of a point on the source polyline (linearly from the start (0) to the end (1)).
        To see if a given curve - 'bez' - is a close approximation of the polyline,
        we compare such a poly-point to the point on the curve that's the same
        relative distance along the curve's length.
        But finding that curve-point takes a little work:
        There is a function "B(t)" to find points along a curve from the parametric parameter 't'
        (also relative from 0 to 1: http://stackoverflow.com/a/32841764/1869660
                                    http://pomax.github.io/bezierinfo/#explanation),
        but 't' isn't linear by length (http://gamedev.stackexchange.com/questions/105230).
        So, we sample some points along the curve using a handful of values for 't'.
        Then, we calculate the length between those samples via plain euclidean distance;
        B(t) concentrates the points around sharp turns, so this should give us a good-enough outline of the curve.
        Thus, for a given relative distance ('param'), we can now find an upper and lower value
        for the corresponding 't' by searching through those sampled distances.
        Finally, we just use linear interpolation to find a better value for the exact 't'.
        More info:
            http://gamedev.stackexchange.com/questions/105230/points-evenly-spaced-along-a-bezier-curve
            http://stackoverflow.com/questions/29438398/cheap-way-of-calculating-cubic-bezier-length
            http://steve.hollasch.net/cgindex/curves/cbezarclen.html
            https://github.com/retuxx/tinyspline
    */
    var lenMax: Double
    var lenMin: Double
    var tMax: Double
    var tMin: Double
    var t: Double = 0.0

    //Find the two t-s that the current param distance lies between,
    //and then interpolate a somewhat accurate value for the exact t:
    for(i in 1..B_parts) {

        if(param <= t_distMap[i]) {
            tMin   = ((i-1) / B_parts).toDouble()
            tMax   = (i / B_parts).toDouble()
            lenMin = t_distMap[i-1]
            lenMax = t_distMap[i]

            t = (param-lenMin)/(lenMax-lenMin) * (tMax-tMin) + tMin
            break
        }
    }
    return t
}





/**
* Creates a vector of length 1 which shows the direction from B to A
*/
fun createTangent(pointA: MutableList<Double>, pointB: MutableList<Double>): MutableList<Double> {
    return normalize(subtract(pointA, pointB));
}


/**
 * Class Maths
 */

/**
Simplified versions of what we need from math.js
Optimized for our input, which is only numbers and 1x2 arrays (i.e. [x, y] coordinates).
*/
//zeros = logAndRun(math.zeros);
fun zeros_Xx2x2(x: Int): MutableList<MutableList<Double>> {
    var zs = mutableListOf<MutableList<Double>>()
    for(i in 0..x) {
        zs.add(mutableListOf(0.0,0.0))
    }
    return zs
}

//multiply = logAndRun(math.multiply);
fun mulItems(items: MutableList<Double>, multiplier: Double): MutableList<Double> {
    return items.map{x -> x*multiplier}.toMutableList()
}
fun mulMatrix(m1: MutableList<Double>, m2: MutableList<Double>): Double {
    //https://en.wikipedia.org/wiki/Matrix_multiplication#Matrix_product_.28two_matrices.29
    //Simplified to only handle 1-dimensional matrices (i.e. arrays) of equal length:
    return m1.reduceIndexed{i, sum, x1 -> sum + (x1*m2[i])}
    // TODO: 20/11/2021 Qui ho rimosso il ,0
}

//Only used to subract to points (or at least arrays):
//  subtract = logAndRun(math.subtract);
fun subtract(arr1: MutableList<Double>, arr2: MutableList<Double>): MutableList<Double> {
    return arr1.mapIndexed{i, x1 -> x1 - arr2[i]}.toMutableList()
}

//add = logAndRun(math.add);
fun addArrays(arr1: MutableList<Double>, arr2: MutableList<Double>): MutableList<Double> {
    return arr1.mapIndexed{i, x1 -> x1 + arr2[i]}.toMutableList()
}
fun addItems(items: MutableList<Double>, addition: Int): MutableList<Double> {
    return items.map{x -> x+addition}.toMutableList()
}

//var sum = logAndRun(math.sum);
fun sum(items: MutableList<Double>): Double {
    return items.reduce{sum, x -> sum + x}
}

//chain = math.chain;

//Only used on two arrays. The dot product is equal to the matrix product in this case:
//  dot = logAndRun(math.dot);
fun dot(m1: MutableList<Double>, m2: MutableList<Double>): Double {
    return mulMatrix(m1, m2)
}

//https://en.wikipedia.org/wiki/Norm_(mathematics)#Euclidean_norm
//  var norm = logAndRun(math.norm);
fun vectorLen(v: MutableList<Double>): Double {
    return hypot(v[0], v[1])
}

//math.divide = logAndRun(math.divide);
fun divItems(items: MutableList<Double>, divisor: Double): MutableList<Double> {
    return items.map{x -> x/divisor}.toMutableList()
}

//var dotPow = logAndRun(math.dotPow);
fun squareItems(items: MutableList<Double>): MutableList<Double> {
    return items.map{x -> x*x}.toMutableList()
}

fun normalize(v: MutableList<Double>): MutableList<Double> {
    return divItems(v, vectorLen(v))
}

//Math.pow = logAndRun(Math.pow);


/**
 * Class Bezier
 */
//Evaluates cubic bezier at t, return point
fun q(ctrlPoly: MutableList<MutableList<Double>>, t: Double): MutableList<Double> {
    var tx = 1.0 - t;
    var pA = mulItems( ctrlPoly[0],      tx * tx * tx )
    var pB = mulItems( ctrlPoly[1],  3 * tx * tx *  t )
    var pC = mulItems( ctrlPoly[2],  3 * tx *  t *  t )
    var pD = mulItems( ctrlPoly[3],       t *  t *  t )
    return addArrays(addArrays(pA, pB), addArrays(pC, pD))
}

//Evaluates cubic bezier first derivative at t, return point
fun qprime(ctrlPoly: MutableList<MutableList<Double>>, t: Double): MutableList<Double> {
    var tx = 1.0 - t;
    var pA = mulItems( subtract(ctrlPoly[1], ctrlPoly[0]),  3 * tx * tx )
    var pB = mulItems( subtract(ctrlPoly[2], ctrlPoly[1]),  6 * tx *  t )
    var pC = mulItems( subtract(ctrlPoly[3], ctrlPoly[2]),  3 *  t *  t )
    return addArrays(addArrays(pA, pB), pC);
}

//Evaluates cubic bezier second derivative at t, return point
fun qprimeprime(ctrlPoly: MutableList<MutableList<Double>>, t: Double): MutableList<Double> {
    return addArrays(
        mulItems( addArrays(subtract(ctrlPoly[2], mulItems(ctrlPoly[1], 2.0)), ctrlPoly[0]),  6 * (1.0 - t) ),
        mulItems( addArrays(subtract(ctrlPoly[3], mulItems(ctrlPoly[2], 2.0)), ctrlPoly[1]),  6 *        t  )
    )
}
