...
//            hueBackgroundBitmap!!.canvas.drawCircle(centerX, centerY, raggio, satValPaint)
    ...
    private fun drawSatValPanel(canvas: Canvas) {
        ...
        var pathTriangolo = Path().apply {
            moveTo(
                satPointA.x, satPointA.y
            )
            lineTo(
                satPointB.x, satPointB.y
            )
            lineTo(
                satPointC.x, satPointC.y
            )
            close()
        }
...
//            satValBackgroundBitmap!!.canvas.drawPath(pathTriangolo, satValPaint)
            satValBackgroundBitmap!!.canvas.drawCircle(centerX, centerY, satRaggio, satValPaint)
    ...
    private fun satValToPoint(sat: Float, `val`: Float): PointF {
        ...
        Log.d(TAG, "satValToPoint: $deltaX $deltaY")

        val p = PointF()
        p.x = satPointB.x + satValRect.left + deltaX * sat
        p.y = satPointC.y + satValRect.top - deltaY * `val`

        Log.d(TAG, "satValToPoint: $p")
